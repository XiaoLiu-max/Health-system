...
public class FriendApplyService {
    ...
    public void sendApply(Long fromUserId, Long toUserId) {
        ...
        // 4. 查询是否已经存在 待处理 的申请
        QueryWrapper<FriendApply> aqWait = new QueryWrapper<>();
        aqWait.eq("from_user_id", fromUserId)
                .eq("to_user_id", toUserId)
                .eq("status", 0);

        long count = friendApplyMapper.selectCount(aqWait);
        if (count > 0) {
            throw new RuntimeException("已发送过好友申请，请勿重复发送");
        }

        // 5. 查询是否有历史申请（拒绝/过期）
        QueryWrapper<FriendApply> aqOld = new QueryWrapper<>();
        aqOld.eq("from_user_id", fromUserId).eq("to_user_id", toUserId);
        FriendApply oldApply = friendApplyMapper.selectOne(aqOld);

        if (oldApply != null) {
            // 有历史记录 → 重新发起申请
            oldApply.setStatus(0);
            oldApply.setApplyTime(LocalDateTime.now());
            oldApply.setHandleTime(null);
            friendApplyMapper.updateById(oldApply);
        } else {
            // 无历史记录 → 新增
            FriendApply apply = new FriendApply();
            apply.setFromUserId(fromUserId);
        ...
        // 发送消息
        messageServices.sendFriendApplyMsg(fromUserId, toUserId, "申请添加你为好友");
    }
